...
export function calcularCuotaDeuda({ saldo, tasaTipo, tasaValor, mesesRestantes, residualPct = 0 }) {
  const pv = Number(saldo || 0)
  const n = Math.max(1, Number(mesesRestantes || 1))
  const tasaMensualValor = tasaTipo === 'nmv' ? tasaDesdeNMV(tasaValor) : tasaMensual(tasaValor)
  const balloon = pv * (Number(residualPct || 0) / 100)

  if (!tasaMensualValor) {
    return Math.round((pv - balloon) / n)
  }

  const discount = Math.pow(1 + tasaMensualValor, -n)
  const cuota = (pv - balloon * discount) * (tasaMensualValor / (1 - discount))
...
export function generarProyeccionDeuda(deuda, mode = 'base', horizonteMeses = 120) {
  const r = deuda.tasaTipo === 'nmv' ? tasaDesdeNMV(deuda.tasaValor) : tasaMensual(deuda.tasaValor)
  const puntos = [{ mes: 0, saldo: Number(deuda.saldo || 0) }]
  let saldo = Number(deuda.saldo || 0)
  const n = Math.max(1, Number(deuda.mesesRestantes || deuda.plazoMeses || 1))
  const balloon = Number(deuda.globo || saldo * (Number(deuda.residualPct || 0) / 100))
  ...
  const horizonte = Math.max(1, Math.min(horizonteMeses, n + 24))
  ...
  for (let mes = 1; mes <= horizonte; mes++) {
    ...
    const capitalBase = Math.max(0, Number(deuda.cuotaMensual || 0) - interes)
    saldo = Math.max(balloon > 0 ? balloon : 0, saldo - capitalBase)

    if (mode === 'plan' || mode === 'real') {
      const abonoMensual = mode === 'plan' ? extraMensualPlan : avgReal
      saldo = Math.max(0, saldo - abonoMensual)
    }

    if (mode === 'plan') {
      const extraPuntual = planificados
        .filter(a => Number(a.mesOffset || 0) === mes)
        .reduce((s, a) => s + Number(a.monto || 0), 0)
      saldo = Math.max(0, saldo - extraPuntual)
    }

    if (mes === n && balloon > 0) saldo = Math.max(0, saldo - balloon)
...
export function proyectarUSDConLineas(inversion, horizonteAnos = 10) {
  ...
  const trm = Number(inversion.trmActual || 1)
  let base = Number(inversion.capitalInicialUSD || 0)
  let plan = Number(inversion.capitalInicialUSD || 0)
  let real = Number(inversion.capitalInicialUSD || 0)
  const aportePlanUSD = Number(inversion.aporteMensualCOP || 0) / trm
  const aporteRealUSD = avgAmount(inversion.aportesRealizados || []) / trm
  const puntos = [{ ano: 0, Base: Math.round(base), Plan: Math.round(plan), Real: Math.round(real) }]
  ...
  for (let m = 1; m <= horizonteAnos * 12; m++) {
    ...
    const extraPlanUSD = (inversion.aportesPlanificados || [])
      .filter(a => Number(a.mesOffset || 0) === m)
      .reduce((s, a) => s + Number(a.monto || 0), 0) / trm
    plan += extraPlanUSD

    if (m % 12 === 0) {
      puntos.push({ ano: m / 12, Base: Math.round(base), Plan: Math.round(plan), Real: Math.round(real) })
    }
  }
...
// abonosExtraordinarios: { mes_numero: monto } — abonos puntuales (bonos, primas)
export function amortizarHipoteca(saldoInicial, tasaEA, cuotaBase, abonoMensualFijo = 0, abonosExtraordinarios = {}) {
  ...
  while (saldo > 0 && meses < 600) {
    ...
    const amortCuota = Math.min(Math.max(0, cuotaTotal - interes), saldo)
    const extra = abonosExtraordinarios[meses] || 0
    interesesTotal += interes
    saldo = Math.max(0, saldo - amortCuota - extra)
    tabla.push({ mes: meses, saldo: Math.round(saldo), interes: Math.round(interes), extra: Math.round(extra) })
  }
...
export function deudaPlanMensual(deuda) {
  const puntual = (deuda.abonosPlanificados || [])
    .filter(a => Number(a.mesOffset || 0) === 1)
    .reduce((s, a) => s + Number(a.monto || 0), 0)
  return Number(deuda.cuotaMensual || 0) + Number(deuda.abonoMensualPlan || 0) + puntual
}
...
export function inversionPlanMensual(inversion) {
  const puntual = (inversion.aportesPlanificados || [])
    .filter(a => Number(a.mesOffset || 0) === 1)
    .reduce((s, a) => s + Number(a.monto || 0), 0)
...
export function inversionRealMensual(inversion, period = currentPeriod()) {
  // aportesRealizados tienen campo 'periodo' (YYYY-MM)
  const deAportes = sumByPeriod(inversion.aportesRealizados || [], period)
  // transacciones tienen campo 'fecha' (YYYY-MM-DD) — convertir a periodo
  const deTransacciones = (inversion.transacciones || [])
    .filter(t => t.fecha && t.fecha.slice(0, 7) === period)
    .reduce((s, t) => s + Number(t.cop || 0), 0)
  return deAportes + deTransacciones
}

// proyectarPatrimonioDesdeEstado
// Incluye: crecimiento salarial, valor de activos físicos, capital USD real desde transacciones
export function proyectarPatrimonioDesdeEstado(state, anos = 10) {
  ...
  // Capital USD real desde transacciones registradas
  const txs = state.inversion?.transacciones || []
  const capitalFromTx = txs.reduce((s, t) => s + Number(t.usd || t.cop / t.trm || 0), 0)
  let usdBase = capitalFromTx || Number(state.inversion.capitalInicialUSD || 0)
  let usdPlan = usdBase
  let trm = Number(state.inversion.trmActual || 0)

  // Crecimiento salarial: tasa anual configurada en ingresos
  ...
  // Activos físicos: valor de mercado estimado (se deprecian 10% anual)
  const activosIniciales = (state.activos || []).reduce((s, a) => s + Number(a.valorMercado || 0), 0)

  // Deudas con balloon tracking
  ...
  const pushPoint = (m) => {
    const deudaBase = deudaStates.reduce((s, d) => s + d.saldoBase, 0)
    const deudaPlan = deudaStates.reduce((s, d) => s + d.saldoPlan, 0)
    const invBaseCop = usdBase * trm
    const invPlanCop = usdPlan * trm
    // Activos se deprecian 10% anual
    const activosAhora = activosIniciales * Math.pow(0.90, m / 12)
    puntos.push({
  ...
  for (let m = 1; m <= anos * 12; m++) {
    ...
    deudaStates.forEach(d => {
      const r = d.tasaTipo === 'nmv' ? tasaDesdeNMV(d.tasaValor) : tasaMensual(d.tasaValor)
      const balloon = Number(d.globo || 0)
      const interesBase = d.saldoBase * r
      const interesPlan = d.saldoPlan * r
      d.saldoBase = Math.max(balloon > 0 ? balloon : 0, d.saldoBase - Math.max(0, Number(d.cuotaMensual || 0) - interesBase))
      d.saldoPlan = Math.max(balloon > 0 ? balloon : 0, d.saldoPlan - Math.max(0, Number(d.cuotaMensual || 0) - interesPlan) - Number(d.abonoMensualPlan || 0))
      const extraPlan = (d.abonosPlanificados || [])
        .filter(a => Number(a.mesOffset || 0) === m)
        .reduce((s, a) => s + Number(a.monto || 0), 0)
      d.saldoPlan = Math.max(0, d.saldoPlan - extraPlan)
    })

    // Crecimiento salarial compuesto → aumenta capacidad de aporte
    ...
    const extraPlanUsd = (state.inversion.aportesPlanificados || [])
      .filter(a => Number(a.mesOffset || 0) === m)
      .reduce((s, a) => s + Number(a.monto || 0), 0) / Math.max(1, trm)
...
export function proyectarPatrimonio(config) {
  ...
  let h = Number(saldo330 || 0), d = Number(saldo288 || 0), usd = Number(capitalUSD || 0), trm = Number(trmBase || 0)
  const puntos = []
  const addPunto = (m) => {
    const deudaTotal = h + d
    const invCOP = usd * trm
    puntos.push({ label: String(2026 + Math.floor(m / 12)), mes: m, deuda: Math.round(deudaTotal / 1e6), inversion: Math.round(invCOP / 1e6), patrimonio: Math.round((invCOP - deudaTotal) / 1e6) })
  }
  addPunto(0)
  for (let m = 1; m <= 240; m++) {
    if (h > 0) { const i = h * tm330; h = Math.max(0, h - Math.max(0, 3700000 + Number(abonoHipoteca || 0) - i)) }
    if (m <= 60 && d > 0) { const i = d * tm288; d = Math.max(0, d - Math.max(0, 5440000 - i)) } else d = 0
...
export function proyectarSalario(salarioBrutoActual, crecimientoAnualPct, anos = 10) {
  ...
  return Array.from({ length: anos }, (_, i) => {
    ...
    const bruto = Math.round(salarioBrutoActual * Math.pow(1 + tasa, i + 1))
    // Estimación retención progresiva (~28% sobre incremento)
    const retencionExtra = Math.round((bruto - salarioBrutoActual) * 0.28)
    return { ano, bruto, retencionExtra }
  })
}

// ── Simulador de compra extraordinaria ────────────────────────────────────
// Retorna cuántos meses afecta el plan y el impacto en inversión USD
export function simularCompra({ precioTotal, financiado, plazoMeses, tasaEA, excedenteMensual, aporteUSDMensual }) {
  ...
  if (!financiado) {
    ...
    const perdidaInversionUSD = aporteUSDMensual * mesesParaAhorrar // oportunidad perdida
    return {
      modo: 'contado',
      cuotaMensual: 0,
      ...
      totalPagado: precio,
      interesesPagados: 0,
      excedenteTrasCompra: exc - precio / mesesParaAhorrar,
      perdidaInversionUSD,
      viable: exc > 0,
    }
  }
  ...
  const perdidaInversionUSD = Math.max(0, cuota) * n // lo que no va a USD
...
export function detectarHitos(state) {
  ...
  // Deudas que se cancelan próximamente
  state.deudas?.forEach(d => {
    if (!d.mesesRestantes || d.saldo <= 0) return
    const fechaFin = new Date(hoy.getFullYear(), hoy.getMonth() + d.mesesRestantes, 1)
    const anioFin = fechaFin.getFullYear()
    const mesesRest = d.mesesRestantes

    if (mesesRest <= 3) {
      hitos.push({ tipo: 'deuda_proxima', urgencia: 'alta', icono: '🔴',
        titulo: `${d.nombre} se cancela en ${mesesRest} mes${mesesRest !== 1 ? 'es' : ''}`,
        detalle: `Se liberan ${fmt(d.cuotaMensual)}/mes de nómina`,
        meses: mesesRest })
    } else if (mesesRest <= 12) {
      hitos.push({ tipo: 'deuda_proxima', urgencia: 'media', icono: '🟡',
        titulo: `${d.nombre} se cancela en ${anioFin}`,
        detalle: `Liberará ${fmt(d.cuotaMensual)}/mes`,
        meses: mesesRest })
    }

    // Globo/residual próximo
    if (d.globo > 0 && mesesRest <= 24) {
      hitos.push({ tipo: 'globo', urgencia: mesesRest <= 6 ? 'alta' : 'media', icono: '⚠',
  ...
  // Portafolio USD milestones
  const txs = state.inversion?.transacciones || []
  const capitalUSD = txs.reduce((s, t) => s + Number(t.usd || t.cop / t.trm || 0), 0)
    || Number(state.inversion?.capitalInicialUSD || 0)

  const milestones = [500, 1000, 5000, 10000, 25000, 50000]
  for (const m of milestones) {
    if (capitalUSD < m && capitalUSD > m * 0.8) {
      hitos.push({ tipo: 'usd_milestone', urgencia: 'info', icono: '💵',
  ...
  // Meta de patrimonio (si está configurada)
  ...
  if (meta?.valorCOP > 0) {
    ...
    if (puntoMeta) {
      const patrimonioEnMeta = puntoMeta.Plan * 1e6
      const pct = Math.round(patrimonioEnMeta / meta.valorCOP * 100)
      if (pct < 80) {
        hitos.push({ tipo: 'meta', urgencia: 'media', icono: '🎯',
        ...
        hitos.push({ tipo: 'meta', urgencia: 'exito', icono: '✅',
  ...
  return hitos.sort((a, b) => a.meses - b.meses)
}
